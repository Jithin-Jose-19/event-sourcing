...
namespace Post.Cmd.Api.Controllers
{
    ...
    public class AddCommentController(ILogger<AddCommentController> logger, ICommandDispatcher commandDispatcher) : ControllerBase
    {
        ...
        [HttpPut("id")]
        public async Task<IActionResult> AddCommentsAsync(Guid id, AddCommentCommand command)
        {
            command.Id = id;
            try
            {
                await _commandDispatcher.SendAsync(command);
                return Ok(new AddCommentResponse
                {
                    Id = id,
            ...
            catch (InvalidOperationException ex)
            {
                ...
                return BadRequest(new AddCommentResponse
                {
                    Id = id,
                    Message = ex.Message
                });
